...
export class FirePit extends BaseLocation {
  ...
  public description(): string {
    ...
    return (
      `You arrive at a wide ring of tamped earth in a clearing. ${fireDesc} ${chairDesc} The oaks are dense enough here that this spot feels genuinely separate from the Lodge and the pool, its own place with its own logic. An empty Topo Chico bottle stands on one of the Adirondack armrests. Someone was here recently.${msgPart}\n\nThe pool deck is to the north. The south meadow is to the northwest. The cedar brake is to the south.` +
      this.appendItems()
    );
  }
