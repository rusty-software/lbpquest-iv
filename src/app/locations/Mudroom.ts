...
export class Mudroom extends BaseLocation {
  ...
  public description(): string {
    ...
    this.items
      ...
      .forEach((i) => (itemText += `\n\n${i.getLocationText()}`));
    return `You are in a utilitarian mud room room between the Great Room and the back porch. A boot rack holds four pairs of boots, none of them yours. Coat hooks line the wall beside it. A washer and dryer sit stacked in the corner.${tallboySentence} A back door leads out to a porch.\n\nThe back porch is to the south. The Great Room is to the north.${itemText}`;
  }
}
