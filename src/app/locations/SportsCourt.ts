...
export class SportsCourt extends BaseLocation {
  ...
  private playPickleball(gameEngine: GameEngine): string {
    ...
    return "You serve to yourself, which is not regulation play but is what is available. The court has good bounce. The net height is correct. You play a set against the back fence, which is a patient opponent. You win, in the technical sense. The scoreboard still reads GERALD: 3 / HUMANS: 0. You do not update it. (5 points)";
  }
  ...
  public customVerbs: Map<string, (gameEngine: GameEngine) => string> = new Map([
    ...
    ["play pickleball", (gameEngine: GameEngine) => this.playPickleball(gameEngine)],
  ]);
}
